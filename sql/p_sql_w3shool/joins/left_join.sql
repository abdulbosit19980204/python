...
-- The LEFT JOIN keyword selects ALL records from the "left" table, and the matching records from
...
-- categories table.
-- By using LEFT JOIN we will get all records from testproducts, even the ones with no match in the
-- categories table:
...
-- OUTER is the default join type for LEFT JOIN, so when you write LEFT JOIN the parse actually
-- writes LEFT OUTER JOIN.
